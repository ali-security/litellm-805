...
class TestAllowedRoutesCallerPermission:
    ...
    def test_safe_presets_allowed_when_flag_set(self):
        from litellm.proxy.management_endpoints.key_management_endpoints import (
        ...
        _check_allowed_routes_caller_permission(
            ["info_routes"], user, allow_safe_presets=True,
        )
        _check_allowed_routes_caller_permission(
            ["llm_api_routes"], user, allow_safe_presets=True,
        )

    def test_safe_presets_blocked_without_flag(self):
        from litellm.proxy.management_endpoints.key_management_endpoints import (
            _check_allowed_routes_caller_permission,
        )
        user = UserAPIKeyAuth(
        ...
        with pytest.raises(HTTPException) as exc_info:
            _check_allowed_routes_caller_permission(["info_routes"], user)
        assert exc_info.value.status_code == 403

    def test_management_routes_blocked_even_with_safe_presets_flag(self):
        from litellm.proxy.management_endpoints.key_management_endpoints import (
            _check_allowed_routes_caller_permission,
        )
        user = UserAPIKeyAuth(
        ...
        with pytest.raises(HTTPException) as exc_info:
            _check_allowed_routes_caller_permission(
                ["management_routes"], user, allow_safe_presets=True,
            )
        assert exc_info.value.status_code == 403
